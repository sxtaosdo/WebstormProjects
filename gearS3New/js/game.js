...
var GameJs = (function () {
    ...
    function autoHideTopIconStop() {
        if (autoHideKey > 0) {
            clearTimeout(autoHideKey);
        }
    }

    function autoHideTopIcon() {
        autoHideTopIconStop();
    ...
    function showTopIcon(key, index) {
        if (index) {
            changeTopIcon(exportRoot.gameView.topIcon["i" + index], key ? 1 : 0)
            exportRoot.gameView.topIcon["i" + index].gotoAndStop(exportRoot.gameView.topIcon["i" + index].totalFrames - 1);
        } else {
            changeAllTopIcon(key ? 1 : 0);
        }
    }

    function changeAllTopIcon(key) {
        for (var i = 1; i < 8; i++) {
            changeTopIcon(exportRoot.gameView.topIcon["i" + i], key);
        }
    }

    function changeTopIcon(target, alpha) {
        var time = 1000;
        if (alpha == 1) {
            time = 200;
        }
        createjs.Tween.removeTweens(target);
        createjs.Tween.get(target).to({alpha: alpha}, time);
    }

    function checkSelectIcon() {
        for (key in currentLevelIndex) {
            if (currentLevelIndex[key] == false) {
                showTopIcon(true, key);
            }
        }
    }
