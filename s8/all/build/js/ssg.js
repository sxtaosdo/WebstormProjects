...
},{"./clipboard-action":1,"good-listener":7,"tiny-emitter":12}],3:[function(require,module,exports){
...
var ExperienceTypeKeys = exports.ExperienceTypeKeys = {};

for (var key in ExperienceType) {
    var experienceType = ExperienceType[key];
    ExperienceTypeKeys[experienceType] = key;
}

exports.default = ExperienceType;
...
var _ExperienceType = require('./ExperienceType');

var _ExperienceType2 = _interopRequireDefault(_ExperienceType);
...
var FeatureDetection = function () {
    ...
    var highExperienceSeries = ['Galaxy S7', 'Galaxy S7 Edge', '6 or 7', '6+ or 7+'];

    var mediumExperienceSeries = ['Galaxy S6', 'Galaxy S6 Edge', 'Galaxy S6 Edge+', 'Galaxy Note 5', '5'];

    var overridenProps = [];

    var uaParser = new UAParser();
    var phone = determinePhoneInfo();
    var experienceType = void 0,
        experienceTypeKey = void 0;
    ...
    function init() {
        ...
        console.info(phone);
        console.log('%c🐞Experience Type%c' + experienceType, 'background: #000; color: #FFF; font-size: 16px; font-weight: bold; padding: 2px 8px; border-radius: 16px 0 0 16px;', 'background: #1428A0; color: #FFF; font-size: 16px; font-weight: bold; padding: 2px 8px; border-radius: 0 16px 16px 0;');
    ...
    function getOsMajorVersion(os) {
        ...
        if (os.name === OS.ANDROID || os.name === OS.IOS) {
            var matches = os.version.match(/^(\d+)/g);
    ...
    function getDeviceSeries(device) {
        ...
        // ::: Samsung Series
        for (var series in seriesPatterns) {
    ...
    function setExperienceType(newExperienceType) {
        ...
        experienceTypeKey = _ExperienceType.ExperienceTypeKeys[experienceType];

        // clear old experience type body class
        for (var expType in _ExperienceType2.default) {
    ...
    return {
        ...
        getExperienceTypeKey: getExperienceTypeKey,
        OS: OS,
        DEVICE: DEVICE,
        BROWSER: BROWSER,
        SOCIAL: SOCIAL
...
},{"./ExperienceType":14,"./utils":19,"ua-parser-js":13}],16:[function(require,module,exports){
'use strict';
...
},{"pubsub-js":10}],17:[function(require,module,exports){
'use strict';
...
var bundles = {
    ...
    // globalAssets: [''],
    infinityDisplay: ['./assets/images/infinityDisplay/grass1.png'],
...
var loadCss = function loadCss(asset, cb) {
...
var loadData = function loadData(responseType, uriData, asset, cb, progressFn, encode64, requestWebp) {
    ...
    xhr.onload = function () {
        ...
        if (!encode64) {
            ...
            for (var i = 0, j = arr.length; i < j; i += 500) {
...
var assets = {
    ...
    load: function load() {
        ...
        var bundleIds = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : ['core', 'globalAssets', 'phoneAssets', 'phoneBreakAnim'];

        var startLoadTime = void 0;

        // first check if all requested bundles are already loaded, remove from requested array those that are
        bundleIds = bundleIds.filter(function (id) {
            return !bundles[id].loaded;
        });

        var loadingBundleIds = bundleIds.filter(function (id) {
        ...
        var totalAssets = bundleIds.reduce(function (a, b) {
            return a.concat(bundles[b]);
        }, []);

        var assetsPercentages = new Array(totalAssets.length),
        ...
        var loadPromise = new Promise(function (resolve, reject) {
            ...
            function getTotalPercentage() {
                ...
                assetsPercentages.forEach(function (perc) {
                    total += perc;
                });
            ...
            var loadedFn = function loadedFn(asset, el) {
                _this.loadedAssets[asset] = el;
                ...
                if (loadedAssetsCount == totalAssetsCount) {
                    ...
                    var loadingBundles = void 0;
                    // check if there are any stacked loadingResolves pending
                    for (var key in loadingResolves) {
                        /*jshint loopfunc: true */
                        loadingBundles = key.split(',');
                        loadingBundles = loadingBundles.filter(function (id) {
                            return bundles[id].loading;
                        });
            ...
            var partialLoadFn = function partialLoadFn(asset, percentage) {
                var rel100 = 1 / totalAssetsCount;
            ...
            totalAssets.forEach(function (asset) {
                ...
                switch (assetType) {
                    ...
                    case 'jpg':
                    case 'png':
                        if (_this.webpSupport) {
                            assetType = "webp";
                        }

                        loadData("image/" + assetType, "image/" + assetType + ";base64", asset, loadedFn, partialLoadFn, true, _this.webpSupport);
                        break;
    ...
    asyncLoadAllBundles: function asyncLoadAllBundles() {
        var nextBundleId = void 0;
    ...
    getBundle: function getBundle(bundleId) {
        var _this2 = this;

        var bundleHash = {};

        bundles[bundleId].forEach(function (asset) {
            bundleHash[asset] = _this2.loadedAssets[asset];
        });

        return bundleHash;
    },
    dispose: function dispose(assetId) {
        this.loadedAssets[assetId] = undefined;
    },
    disposeBundle: function disposeBundle(bundleId) {
        var bundleAssets = this.getBundle(bundleId);

        for (var asset in bundleAssets) {
            this.dispose(asset);
        }
    },
    setBundleDomSrc: function setBundleDomSrc(bundleId, domEl, force, dispose) {
        var _this3 = this;

        var assetEls = domEl.querySelectorAll('[asset-src], [asset-src-background]');
        ...
        Array.prototype.slice.call(assetEls).forEach(function (el) {
            ...
            if (el.hasAttribute('asset-src-background') && (!el.hasAttribute('asset-background-set') || force)) {
                el.style.backgroundImage = 'url(' + _this3.loadedAssets[el.getAttribute('asset-src-background')] + ')';
                el.setAttribute('asset-background-set', true);
            }

            if (dispose) {
                _this3.dispose[el.getAttribute('asset-src') || el.getAttribute('asset-src-background')]; // jshint ignore:line
            }
        });
    }
};
...
exports.floorRandomBetween = floorRandomBetween;
exports.degreesBetween = degreesBetween;
exports.limit = limit;
exports.injectOpacity = injectOpacity;
...
var isDeviceOrientationSupported = exports.isDeviceOrientationSupported = !!window.DeviceOrientationEvent;

var isDeviceMotionSupported = exports.isDeviceMotionSupported = window.hasOwnProperty('ondevicemotion');

// taken from https://github.com/mrdoob/three.js/blob/master/examples/js/Detector.js
var isWebglSupported = exports.isWebglSupported = function () {
    try {
...
var isClipboardSupported = exports.isClipboardSupported = window.Clipboard.isSupported();
...
function injectOpacity(rgba, opacity) {
...
function addEventListenerToNodeList(nodeList, event, handler) {
...
function findKeyframesRule(rule) {
    ...
    for (var i = 0; i < stylesheets.length; ++i) {
        ...
        for (var j = 0; j < stylesheets[i].cssRules.length; ++j) {
            ...
            if (stylesheets[i].cssRules[j].type == (CSSRule.WEBKIT_KEYFRAMES_RULE || CSSRule.KEYFRAMES_RULE) && stylesheets[i].cssRules[j].name == rule) return stylesheets[i].cssRules[j];
        }
    }
...
var ShootingStar = function () {
    function ShootingStar(domEl, config) {
    ...
    _createClass(ShootingStar, [{
        ...
        value: function animate(params) {
            ...
            setTimeout(function () {
                var values = Object.assign(_this.animateConfig, params);
                ...
                _this.keyframe.deleteRule('0%');
                _this.keyframe.deleteRule('50%');
                _this.keyframe.deleteRule('90%');
                _this.keyframe.deleteRule('100%');

                rule = 'translate(' + values.startX + 'px, ' + values.startY + 'px) scale(0) rotateZ(' + angle + ');';
                if (_this.keyframe.appendRule) {
                    _this.keyframe.appendRule('0% { -webkit-transform: ' + rule + ' transform: ' + rule + ' opacity: 0;}');
                } else {
                    _this.keyframe.insertRule('0% { -webkit-transform: ' + rule + ' transform: ' + rule + ' opacity: 0;}');
                }
                ...
                if (_this.keyframe.appendRule) {
                    _this.keyframe.appendRule('50% { -webkit-transform: ' + rule + ' transform: ' + rule + ' opacity: 1;}');
                } else {
                    _this.keyframe.insertRule('50% { -webkit-transform: ' + rule + ' transform: ' + rule + ' opacity: 1;}');
                }
                ...
                rule = 'translate(' + distX + 'px, ' + distY + 'px) scale(' + values.scale + ') rotateZ(' + angle + ');';
                if (_this.keyframe.appendRule) {
                    _this.keyframe.appendRule('90% { -webkit-transform: ' + rule + ' transform: ' + rule + ' opacity: 1;}');
                } else {
                    _this.keyframe.insertRule('90% { -webkit-transform: ' + rule + ' transform: ' + rule + ' opacity: 1;}');
                }

                rule = 'translate(' + values.endX + 'px, ' + values.endY + 'px) scale(' + values.scale + ') rotateZ(' + angle + ');';
                if (_this.keyframe.appendRule) {
                    _this.keyframe.appendRule('100% { -webkit-transform: ' + rule + ' transform: ' + rule + ' opacity: 0;}');
                } else {
                    _this.keyframe.insertRule('100% { -webkit-transform: ' + rule + ' transform: ' + rule + ' opacity: 0;}');
                }

                _this.el.style.animationDuration = _this.el.style.webkitAnimationDuration = values.duration + 's';
                _this.el.style.animationDelay = _this.el.style.webkitAnimationDelay = values.delay + 's';
                _this.el.style.animationName = _this.el.style.webkitAnimationName = _this.animateConfig.keyframeId;
            }, 0);
        }
    }]);
...
},{"clipboard":2,"es6-promise":5}],20:[function(require,module,exports){
'use strict';
...
var nav = function () {
    ...
    function getElements() {
        ...
        Array.prototype.slice.call(elements.navPills).forEach(function (navPill, index) {
    ...
    function clearActiveNavPill() {
        var pill = getActiveNavPill();
        var pillFill = getActiveNavPillFill();
    ...
    function clearFullNavPills() {
        var fullNavPills = Array.prototype.slice.call(document.querySelectorAll('.nav-pills__link.is-full'));

        for (var i = 0; i < fullNavPills.length; i++) {
    ...
    function setActiveNavPill(navId) {
        ...
        var pill = getNavPillById(navId);
        var pillFill = getNavPillFillById(navId);

        if (pill) {
            var navIndex = parseInt(pill.getAttribute('data-nav-index'), 10);
            console.info('[ nav ] setActiveNavLink() ::: navIndex: ', navIndex);

            for (var i = 0; i < navIndex; i++) {
                var previousNavLink = getNavPillByIndex(i);
...
var _PubSub = require('../../common/PubSub');
...
function ProgressBar() {
    ...
    this.loadProgressSubscription = _PubSub2.default.subscribe(_PubSub.messages.APP.LOAD_PROGRESS, this.setProgress.bind(this));
}
...
ProgressBar.prototype = {
    ...
    setProgress: function setProgress(topic, data) {
    ...
    destroy: function destroy() {
        ...
        _PubSub2.default.unsubscribe(this.loadProgressSubscription);
    }
};
...
function initProgressBar() {
    ...
    _PubSub2.default.unsubscribe(initSubscription);
}

var initSubscription = _PubSub2.default.subscribe(_PubSub.messages.APP.INIT_LOAD, initProgressBar);

exports.default = ProgressBar;

},{"../../common/PubSub":16}],22:[function(require,module,exports){
'use strict';
...
function Share(selector) {
    ...
    var $clipboardButton = $container.find('.share__button[data-shareid="clipboard"]');
    var $clipboardSuccessMessage = $container.find('.share__clipboard-success');
    ...
    for (var i = 0; i < this.shareButtons.length; i++) {
        var button = this.shareButtons[i];
    ...
    for (var _i = 0; _i < this.preorderButtons.length; _i++) {
        var _button = this.preorderButtons[_i];
        _button.addEventListener('click', this.handlePreOrderClick, false);
    }

    // ::: Add event listeners

    // ::: Clipboard setup
    if (_utils.isClipboardSupported) {
...
Share.prototype = {
    setupToggleTimeline: function setupToggleTimeline() {
        var _this = this;
        ...
        this.toggleTimeline.staggerFrom(this.buttons, 0.167, {
            ...
            TweenLite.delayedCall(2, _this.toggleTimeline.reverse, [], _this.toggleTimeline);
        });
    },
    shareLinkSuccessTimeline: function shareLinkSuccessTimeline() {
    ...
    handleShareLinkSuccess: function handleShareLinkSuccess() {
    ...
    handlePreOrderClick: function handlePreOrderClick(event) {
...
},{"../../common/utils":19}],23:[function(require,module,exports){
'use strict';
...
var _ExperienceType = require('../common/ExperienceType');

var _ExperienceType2 = _interopRequireDefault(_ExperienceType);

var _navPills = require('./global/navPills');

var _navPills2 = _interopRequireDefault(_navPills);

var _share = require('./global/share');
...
var preOrder = function () {
    ...
    function init(topic, experienceType) {
        ...
        // create stars
        stars.push(new _utils.ShootingStar(document.querySelector('#preOrder .background'), {
            keyframeId: 'star1',
            duration: (0, _utils.randomBetween)(2, 5, 2),
            startX: -40,
            startY: (0, _utils.floorRandomBetween)(30, window.innerHeight * 0.6),
            scale: (0, _utils.randomBetween)(0.05, 0.1, 2),
            endX: (0, _utils.floorRandomBetween)(window.innerWidth * 0.7, window.innerWidth * 1.2),
            endY: (0, _utils.floorRandomBetween)(0, window.innerHeight * 0.4),
            delay: (0, _utils.randomBetween)(1, 2.667, 2)
        }));
        stars.push(new _utils.ShootingStar(document.querySelector('#preOrder'), {
            keyframeId: 'star2',
            duration: (0, _utils.randomBetween)(2, 5, 2),
            startX: window.innerWidth + 200,
            startY: (0, _utils.floorRandomBetween)(30, window.innerHeight * 0.6),
            scale: (0, _utils.randomBetween)(0.05, 0.1, 2),
            endX: (0, _utils.floorRandomBetween)(-40, window.innerWidth * 0.3),
            endY: (0, _utils.floorRandomBetween)(-80, window.innerHeight * 0.4),
            delay: (0, _utils.randomBetween)(0.5, 1.3, 2)
        }));

        if (experienceType === _ExperienceType2.default.LOW) {
            enter();
        } else {
    ...
    function getElements() {
        ...
        var backgroundImage = document.querySelector('#preOrder .backgroundImage');
        ...
        if (backgroundImage.offsetWidth < window.innerWidth) {
            var bgScale = window.innerWidth / backgroundImage.offsetWidth;
        ...
        share = new _share2.default('#preOrder .share');
    }
    ...
    function setupEnterTimeline() {
        ...
        enterTimeline.staggerFrom([elements.phone, elements.lockup, elements.cta, elements.share], 1, {
            ease: Sine.easeInOut,
            autoAlpha: 0
        }, 0.205, 0);
    }
    ...
    function handlePreorderButtonClick(event) {
        ...
        var tapX = event.offsetX + 'px';
        var tapY = event.offsetY + 'px';

        createButtonTapRipple(event.target, tapX, tapY, function () {
    ...
    function createButtonTapRipple(container, x, y, completeCallback) {
        ...
        var tapRipple = document.createElement('div');
        tapRipple.classList.add('tap-ripple');
        container.appendChild(tapRipple);

        // animate
        var timeScale = 1;
        var fillTime = 0.333 * timeScale;
        var fadeTime = 0.205 * timeScale;
    ...
    function enter() {
        _navPills2.default.fillActivePill(100, 2, hideNavPills);

        SSG.assets.load(['globalAssets']).then(function () {
    ...
    function hideNavPills() {
        TweenLite.delayedCall(1, _navPills2.default.hidePills);
    }
...
},{"../common/ExperienceType":14,"../common/utils":19,"./global/navPills":20,"./global/share":22}],24:[function(require,module,exports){
'use strict';
...
var _FeatureDetection = require('./common/FeatureDetection');
...
require('./modules/global/progressBar');
...
window.SSG = {
    ...
    messages: _PubSub.messages,
    assets: _assets2.default,
    FeatureDetection: _FeatureDetection2.default,
    enableNavToggleButton: function enableNavToggleButton() {
        SSG.PubSub.publish(SSG.messages.APP.ENABLE_NAV_TOGGLE_BUTTON);
    },
    disableNavToggleButton: function disableNavToggleButton() {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var bundle = void 0;
    var experienceType = _FeatureDetection2.default.getExperienceType();
    var phone = _FeatureDetection2.default.getPhoneInfo();

    // Fix for SMSNGDREAM-67
    switch (phone.browser.social) {
        case _FeatureDetection2.default.SOCIAL.FACEBOOK:
        case _FeatureDetection2.default.SOCIAL.FACEBOOK_MESSENGER:
        case _FeatureDetection2.default.SOCIAL.INSTAGRAM:
    ...
    _PubSub2.default.publish(_PubSub.messages.APP.INIT_LOAD);

    console.debug('>>> APP INIT LOAD ***%c ssg loaded: ' + (new Date().getTime() - startTime), 'font-weight: bolder; color: #C00;');

    _assets2.default.init().then(function () {
        document.body.classList.add(_assets2.default.webpSupport ? 'webp' : 'no-webp');

        _assets2.default.load(bundle).then(function (loadingTime) {
            console.debug('>>> APP LOAD ***%c bundle loaded: ' + (new Date().getTime() - startTime), 'font-weight: bolder; color: #C00;');

            setTimeout(function () {
                _PubSub2.default.publish(_PubSub.messages.APP.INITIAL_LOAD_COMPLETE, experienceType);
            }, experienceType === _ExperienceType2.default.LOW ? 0 : 2500 - loadingTime);
        });
    });
});
...
},{"./common/ExperienceType":14,"./common/FeatureDetection":15,"./common/PubSub":16,"./common/assets":17,"./common/polyfills":18,"./modules/global/progressBar":21,"./modules/preOrder":23,"npm-zepto":8}]},{},[24]);
